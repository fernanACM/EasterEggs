...
class EventsSubCommand extends BaseSubCommand{

    public function __construct(){
        parent::__construct("events", "", []);
        $this->setPermission(Perms::EVENTS_SUBCOMMAND);
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void{
        ...
        if(!$sender->hasPermission(Perms::EVENTS_SUBCOMMAND)){
            Language::isError($sender, LangKey::ERROR_NO_PERMISSION);
            return;
        }
        EE::getInstance()->getFormManager()->event()->open($sender);
